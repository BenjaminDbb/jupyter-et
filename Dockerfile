...
RUN apt-get -qq update && \
    apt-get -qq install --no-install-recommends \
        build-essential python python3-pip gcc g++ make gfortran git \
        subversion curl gnuplot gnuplot-x11 time libopenmpi-dev patch \
        libhdf5-openmpi-dev openmpi-bin procps unzip mercurial \
        libnuma-dev numactl hwloc libhwloc-dev libhdf5-dev libjpeg-turbo?-dev libssl-dev \
        hdf5-tools gdb gsl-bin libgsl0-dev python3-setuptools \
        ffmpeg libgsl-dev libopenblas-dev libpapi-dev fftw3-dev libfftw3-dev \
